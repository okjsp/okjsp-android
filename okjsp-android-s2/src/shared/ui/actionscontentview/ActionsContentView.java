...
public class ActionsContentView extends ViewGroup {
    ...
    public ActionsContentView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mSpacingType = a.getInteger(R.styleable.ActionsContentView_spacing_type,
                SPACING_RIGHT_OFFSET);
        final int spacingDefault = context.getResources().getDimensionPixelSize(
        ...
        final int actionsSpacingDefault = context.getResources().getDimensionPixelSize(
                R.dimen.default_actionscontentview_actions_spacing);
        mActionsSpacing = a.getDimensionPixelSize(R.styleable.ActionsContentView_actions_spacing,
        ...
        mFadeValue = (int) a
                .getInt(R.styleable.ActionsContentView_fade_max_value, fadeValueDefault);
        ...
        mFlingDuration = a.getInteger(R.styleable.ActionsContentView_fling_duration,
                flingDurationDefault);

        mSwipeType = a.getInteger(R.styleable.ActionsContentView_swiping_type, SWIPING_EDGE);
        final int swipingEdgeWidthDefault = context.getResources().getDimensionPixelSize(
                R.dimen.default_actionscontentview_swiping_edge_width);
        mSwipeEdgeWidth = a.getDimensionPixelSize(
                R.styleable.ActionsContentView_swiping_edge_width, swipingEdgeWidthDefault);
        ...
        addView(viewActionsContainer, new LayoutParams(LayoutParams.MATCH_PARENT,
        ...
        final LinearLayout.LayoutParams shadowParams = new LinearLayout.LayoutParams(mShadowWidth,
        ...
        addView(viewContentContainer, new LayoutParams(LayoutParams.MATCH_PARENT,
                LayoutParams.MATCH_PARENT));
    }
        ...
        for (int i = 0; i < childrenCount; ++i) {
            ...
            if (v == viewActionsContainer) {
                ...
                if (mSpacingType == SPACING_ACTIONS_WIDTH)
                    viewActionsContainer.measure(
                            MeasureSpec.makeMeasureSpec(mSpacing, MeasureSpec.EXACTLY),
                            heightMeasureSpec);
                else
                    // all other situations are handled as SPACING_RIGHT_OFFSET
                    viewActionsContainer.measure(
                            MeasureSpec.makeMeasureSpec(width - mSpacing, MeasureSpec.EXACTLY),
                            heightMeasureSpec);
            } else if (v == viewContentContainer) {
                final int shadowWidth = isShadowVisible() ? mShadowWidth : 0;
                final int contentWidth = MeasureSpec.getSize(widthMeasureSpec) - mActionsSpacing
                        + shadowWidth;
                v.measure(MeasureSpec.makeMeasureSpec(contentWidth, MeasureSpec.EXACTLY),
                        heightMeasureSpec);
            } else {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childrenCount; ++i) {
            ...
            if (v == viewContentContainer) {
                final int shadowWidth = isShadowVisible() ? mShadowWidth : 0;
                v.layout(mActionsSpacing - shadowWidth, 0, mActionsSpacing + v.getMeasuredWidth(),
                        v.getMeasuredHeight());
            } else {
