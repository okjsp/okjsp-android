...
public class DiskLruCache {
    ...
    public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        ...
        if (cacheDir.isDirectory() && cacheDir.canWrite() && ImageLoaderUtils.getUsableSpace(cacheDir) > maxByteSize) {
        	...
        	Log.e(TAG, "openCache(" + cacheDir.getPath() + ", " + formatter.format(maxByteSize) + ")"
        			+ "\n    isDir      :" + cacheDir.isDirectory()
        			+ "\n    canWrite   :" + cacheDir.canWrite()
        			+ "\n    UsableSpace:" + ImageLoaderUtils.getUsableSpace(cacheDir));
        	return new DiskLruCache(cacheDir, maxByteSize);
        }
    }
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath =
                Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED ||
                        !ImageLoaderUtils.isExternalStorageRemovable() ?
                        		ImageLoaderUtils.getExternalCacheDir(context).getPath() :
                        context.getCacheDir().getPath();

        if (DEBUG_LOG) {
        	Log.d(TAG, "getDiskCacheDir(" + uniqueName + "): " + cachePath + File.separator + uniqueName);
        }
